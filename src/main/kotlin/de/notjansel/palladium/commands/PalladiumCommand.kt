...
class PalladiumCommand : TabExecutor, CommandExecutor{
                ...
                "info" -> {
                    if (p0.hasPermission("palladium.info") || p0.hasPermission("*") || p0.isOp) {
                        val textComponent = Component.text()
                            .append(MiniMessage.miniMessage().deserialize("<yellow> ----------[ Palladium ]----------\n"))
                            .append(MiniMessage.miniMessage().deserialize("<yellow>Version: <#32cd32>${Palladium.Things.version} ${isUpdateOrDev()}\n"))
                            .append(MiniMessage.miniMessage().deserialize("<yellow>GitHub: <blue><click:open_url:https://github.com/NotJansel/Palladium><hover:show_text:'<blue>Click to open the Repository in your browser'>Click here</hover></click>\n"))
                            .append(MiniMessage.miniMessage().deserialize("<yellow>Author: <#32cd32>NotJansel\n"))
                            .append(MiniMessage.miniMessage().deserialize("<yellow>Contributors: <#32cd32>None yet! Help the Project to be better by committing some code!\n"))
                            .append(MiniMessage.miniMessage().deserialize("<yellow>Sponsor links: <#FF4800><click:open_url:https://ko-fi.com/jansel>Ko-Fi</click>\n"))
                            .append(MiniMessage.miniMessage().deserialize("<yellow> ----------[ Palladium ]----------"))
                        audience.sendMessage(textComponent.build())
                    } else {
                        audience.sendMessage(MiniMessage.miniMessage().deserialize(ErrorMessages().missingPermission))
                    }
                }
    ...
    private fun isUpdateOrDev(): String {
